...
> **v3.9 里程碑**: 短期目标（原 v3.1）中的 devtmpfs、/proc/self/maps、45 个 POSIX 系统调用已全部实现。TCP/IP 基础协议栈已就绪。

### 短期（v4.0）
- **squashfs**: 只读压缩文件系统，用于 rootfs 镜像
- **sysfs**: 内核对象信息导出（设备、总线、驱动）
- **内核模块独立编译**: 支持 `.km` 格式模块，类似 CoolPotOS 模块系统
- **更多 POSIX 系统调用**: 扩展至 50+ 系统调用
- **UDP/DNS 协议**: 完善网络协议栈上层协议

### 中期（v4.5）
- **NVMe 驱动**: 内核模块形式的 NVMe SSD 驱动
- **多架构支持**: 初步支持 riscv64 架构
- **完整网络协议栈**: HTTP/ARP/DHCP 客户端
